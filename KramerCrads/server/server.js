...
app.post("/addNewCard", (req, res) => {
  let rawdata = fs.readFileSync('data/cards.json');
  let cards = JSON.parse(rawdata);
  let size = cards['cards'].length;
  if(size==0)
  {
    cards['cards'].push(req.body);
    cards['cards'][size]['id'] = "1";
  }
  else
  {
    let max_id = 0
    ...
    cards['cards'].push(req.body);
    cards['cards'][size]['id'] = max_id;
  }
   
  var colors = JSON.parse(fs.readFileSync('data/colors.json'));
  var random_color =""
  let color_exists =true;
  while(color_exists)
  {
    random_color = colors[Math.floor(Math.random() * colors.length)]['name'];
    color_exists=false;
    cards['cards'].forEach(element => {
      if(element['color']==random_color)
        color_exists=true;
    });
  }

  cards['cards'][size]['color'] = random_color;
